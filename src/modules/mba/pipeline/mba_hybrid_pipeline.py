...
and added GMAT/GRE/MBA filtering + stricter, non-generic strengths/recs.
"""
...
def has_strong_test_score(resume_text: str) -> bool:
    ...
    patterns = [
        ...
        r"\bpgp\s*\(mba\)",
        r"post[- ]graduate program(me)? in management",
...
def call_gemini(
    ...
    try:
        ...
        # Extract text from content.parts with proper error handling
        try:
        ...
        # Warn if truncated but allow if we got some text
        if finish_reason == "MAX_TOKENS":
    ...
    for provider_name, provider_func in providers:
        ...
        for attempt in range(max_attempts):
            try:
                ...
                # Don't retry MAX_TOKENS errors - they need higher limits
...
- Every strength and suggestion must reference at least one concrete detail from the resume
  (role, company, function, metric, project, or industry). Avoid generic advice that could apply to anyone.
...
def extract_strengths_and_improvements(resume_text: str) -> dict:
    ...
    try:
        ...
        # 🔐 If strong GMAT/GRE or MBA/PGP already present, drop test-prep recs
...
def run_pipeline(resume_text: str, include_improvement: bool = True) -> dict:
    ...
    print("\n" + "="*60, file=sys.stderr)
    print("MBA RESUME ANALYSIS PIPELINE v4.2", file=sys.stderr)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="MBA Resume Analysis Pipeline v4.2")
