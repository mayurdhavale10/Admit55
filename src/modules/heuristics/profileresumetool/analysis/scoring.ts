import {
  ...
  PERSONA_WEIGHTS,
  type PersonaKey,
} from "@src/modules/heuristics/profileresumetool/analysis/constants";

import {
  BandZ,
  type Band,
  type DimensionScore,
} from "@src/modules/schemas/profileresumetool/evaluation";

import type { MetricsAnalysis } from "@src/modules/heuristics/profileresumetool/content/patterns/detectors/metrics";
import type { VerbsAnalysis } from "@src/modules/heuristics/profileresumetool/content/patterns/detectors/verbs";
import type { LengthAnalysis } from "@src/modules/heuristics/profileresumetool/content/patterns/detectors/length";
import type { KeywordsAnalysis } from "@src/modules/heuristics/profileresumetool/content/patterns/detectors/keywords";
import type { DedupeAnalysis } from "@src/modules/heuristics/profileresumetool/content/patterns/detectors/dedupe";
import type { ConsistencyAnalysis } from "@src/modules/heuristics/profileresumetool/content/patterns/detectors/consistency";
...
/** Map penalty ratio (0–1) to score 0–5 */
...
/** Map total readiness (0–100) to qualitative band */
...
/**
 * Compute dimension-level and aggregate readiness scores.
 */
export function scoreDimensions(input: ScoringInputs): {
  ...
  const passiveAvg = passiveVals.length
    ? passiveVals.reduce((a, b) => a + b, 0) / passiveVals.length
    : 0;
  ...
  const d03 = clamp(
    Math.round((densityToScore(1 - passiveAvg) + d01) / 2),
    0,
    5
  );

  // D04 Progression (placeholder): penalize if date overlaps exist
  const hasOverlap = (consistency.issues ?? []).some(
    (i) => i.type === "date_overlap"
  );
  const d04 = hasOverlap ? 2 : 3; // neutral baseline until real calc

  // Baseline placeholders (2–3) for remaining dimensions
  ...
  const dimensions: DimensionScore[] = DIMENSION_IDS.map((id) => ({
    ...
    evidence: [], // optional signal references later
  }));
