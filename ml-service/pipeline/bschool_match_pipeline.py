...
def build_match_prompt(candidate_profile: Dict[str, Any]) -> str:
  ...
  IMPORTANT:
  We use .replace instead of .format because MATCH_PROMPT_TEMPLATE contains
  many { ... } braces in the example JSON. Using .format() would treat
  those as format placeholders and crash with KeyError like '\\n  "summary"'.
  """
  ...
  # Safely inject just this one placeholder
  prompt = MATCH_PROMPT_TEMPLATE.replace(
